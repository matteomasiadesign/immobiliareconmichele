...
function propertyCardHTML(p) {
    ...
    <div class="card" data-id="${escapeHtml(p.id)}">
      <div class="card-img-container">
        ...
        ${p.reel_url ? `<a href="${escapeHtml(p.reel_url)}" target="_blank" rel="noopener" onclick="event.stopPropagation()" class="card-badge-reel" title="Guarda il Reel">
        ...
        ${images.length > 1 ? `<img src="${escapeHtml(images[1])}" class="card-img card-img-hover" alt="" loading="lazy">` : ''}
      </div>
...
// Il badge del Reel chiama event.stopPropagation(), quindi continua a non
// aprire la scheda.
