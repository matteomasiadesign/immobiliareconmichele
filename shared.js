...
function openModal(id) {
    ...
    setTxt('mod-type', p.property_type || '');
    setTxt('mod-title', p.title || '');
    setTxt('mod-zone', p.zone || '');
    setTxt('mod-rooms', p.rooms ?? '');
    setTxt('mod-bathrooms', p.bathrooms ?? '');

    const modGrid = document.getElementById('mod-grid');
    const sqmItem = document.getElementById('mod-sqm-item');
    if (sqmItem) {
        if (p.sqm) {
            setTxt('mod-sqm', p.sqm);
            sqmItem.style.display = '';
            if (modGrid) modGrid.classList.add('has-sqm');
        } else {
            sqmItem.style.display = 'none';
            if (modGrid) modGrid.classList.remove('has-sqm');
        }
    }

    setTxt('mod-parking', p.has_parking ? "Sì" : "No");
    ...
    setTxt('mod-desc', p.description || '');

    const isReserved = !p.price || Number(p.price) <= 0;
    const priceTxt = isReserved
        ? "Trattativa riservata"
        : ("€" + Number(p.price).toLocaleString('it-IT') + (p.status === 'Affitto' ? " /mese" : ""));
    setTxt('mod-price', priceTxt);
    ...
    if (actionsContainer) {
      ...
      `;
        const shareBtn = actionsContainer.querySelector('.btn-share');
        if (shareBtn) {
            shareBtn.addEventListener('click', () => shareProperty(p.id, p.title));
        }
    }
